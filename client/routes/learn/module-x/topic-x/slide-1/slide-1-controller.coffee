...
Router.map ->
  @route templateName,
    path: "/learn/module-x/topic-x/slide-1"
  return
        ...
        $.each children, (key, value) ->
          ...
          while i < items.length
            if items[i].japanese is childName
              if items[i].english isnt basketName
                valid = false
            i++

    if valid
      $('#createNewModal').modal('show')
      Session.set("modalTitle", "That was correct")
      Session.set("modalBody", "Well done!")
      Session.set("modalButtonText", "Next")
